...
(function () {
  ...
  window.debugMap = {
    ...
    runFullDiagnostic: function () {
      ...
      results.push({
        ...
        details:
          typeof L !== "undefined" ? "Leaflet loaded" : "Leaflet not found",
      });
    ...
    displayDiagnostic: function (results) {
      ...
      let html =
        '<div style="margin-bottom: 10px;"><strong>Diagnostic Results:</strong></div>';
  ...
  if (window.location.pathname.includes("find.html")) {
    ...
    // Auto-show debug panel only in localhost development
    if (
  ...
  console.log(
    "🔧 Debug utility ready. Use window.debugMap or window.showMapDebug()",
  );
})();
